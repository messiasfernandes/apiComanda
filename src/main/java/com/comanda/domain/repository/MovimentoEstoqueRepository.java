...
public interface MovimentoEstoqueRepository extends JpaRepository<EstoqueMovimento, Long>, EstoqueMovmentoQuery {
	...
	@Query(value = "SELECT DISTINCT em FROM EstoqueMovimento em " + "LEFT JOIN FETCH em.items i "
			+ "LEFT JOIN FETCH em.tipoMovimentacaoEstoque t " + "LEFT JOIN FETCH i.produto p "
			+ "LEFT JOIN FETCH p.estoque e " + "LEFT JOIN FETCH p.marca m  " + "LEFT JOIN FETCH p.preco pr "
			+ "LEFT JOIN FETCH p.subgrupo s " + "LEFT JOIN FETCH s.grupo g  " + "WHERE (p.nome LIKE %:parametro% ) "
	...
	Page<EstoqueMovimento> listar(@Param("parametro") String parametro, @Param("tipo") Operacao tipo,
			@Param("dataInicio") LocalDate dataInicio, @Param("dataFim") LocalDate dataFim, Pageable pageable);
